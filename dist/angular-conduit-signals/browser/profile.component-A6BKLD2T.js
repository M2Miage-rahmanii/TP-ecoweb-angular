...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ProfileComponent, [{
    type: Component,
    args: [{ selector: "app-profile", imports: [NgIf, RouterOutlet, ArticleToggleComponent], changeDetection: ChangeDetectionStrategy.OnPush, providers: [provideComponentStore(ProfileStore)], template: `<ng-container *ngIf="profile() as profile">\r
  <div class="banner">\r
    <div class="container">\r
      <div class="user-info">\r
        <img alt="avatar" class="avatar" [src]="profile.image" />\r
        <h4 class="username">{{ profile.username }}</h4>\r
      </div>\r
      <button\r
        (click)="toggleFollow()"\r
        *ngIf="!isCurrentUser(); else editProfileBtn"\r
        class="btn btn-outline-secondary btn-sm ms-auto d-flex align-items-center"\r
      >\r
        <i class="fa-solid fa-plus me-1"></i> {{profile.following ? 'Unfollow' : 'Follow'}} {{ profile.username }}\r
      </button>\r
      <ng-template #editProfileBtn>\r
        <button\r
          class="btn btn-outline-secondary btn-sm ms-auto d-flex align-items-center"\r
          routerLink="/settings"\r
        >\r
          <i class="fa-solid fa-gear me-1"></i> Edit Profile Settings\r
        </button>\r
      </ng-template>\r
    </div>\r
  </div>\r
  <div class="content">\r
    <div class="offset-md-1">\r
      <app-article-toggle></app-article-toggle>\r
...
//# sourceMappingURL=profile.component-A6BKLD2T.js.map
